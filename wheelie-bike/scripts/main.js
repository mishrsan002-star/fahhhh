...
'use strict';
{
    if (window["C3_IsSupported"]) {
        ...
        window["c3_runtimeInterface"] = new self.RuntimeInterface({
            ...
            engineScripts: ["https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@b3070c95a60c28bedb1804e951c47c6cc4999d61/wheelie-bike/scripts/c3runtime.js"],
            projectScripts: [
                ["scripts/project/scriptsInEvents.js"]
            ],
            mainProjectScript: "",
            scriptFolder: "https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@b3070c95a60c28bedb1804e951c47c6cc4999d61/wheelie-bike/scripts/",
...
'use strict';
{
    const DOM_COMPONENT_ID = "iframe";
    const HANDLER_CLASS = class IframeDOMHandler extends self.DOMElementHandler {
        constructor(iRuntime) {
            super(iRuntime, DOM_COMPONENT_ID);
            this._isDisplayingHtmlStr = false;
            this.AddDOMElementMessageHandler("navigate-url", (elem, e) => this._OnNavigateURL(elem, e));
            this.AddDOMElementMessageHandler("display-html", (elem, e) => this._OnDisplayHTML(elem, e))
        }
        CreateElement(elementId, e) {
            ...
            if (/iphone|ipad|ipod/i.test(ua)) {
                const verResult = /(iphone\s+os|ipad[^)]*os)\s+([0-9_]+)/i.exec(ua);
                const iOSVersion = parseInt(verResult[verResult.length - 1]);
                if (iOSVersion <= 12 || isNaN(iOSVersion)) {
                    iframe.setAttribute("noresize", "noresize");
                    iframe.setAttribute("scrolling", "no")
                }
            }
            if (!e["isVisible"]) iframe.style.display = "none";
            if (e["id"]) iframe.id = e["id"];
            if (e["allow"]) {
                iframe.setAttribute("allow", e["allow"]);
                if (e["allow"].includes("fullscreen")) iframe.setAttribute("allowfullscreen",
                    "")
            }
            if (e["enableSandbox"]) iframe.setAttribute("sandbox", e["sandbox"]);
            if (e["url"]) iframe.src = e["url"];
            else if (e["html"]) this._OnDisplayHTML(iframe, e);
            return iframe
        }
        UpdateState(iframe, e) {}
        _SetSrc(iframe, url) {
            if (iframe.src && iframe.src.startsWith("blob:") && this._isDisplayingHtmlStr) URL.revokeObjectURL(iframe.src);
            iframe.src = url
        }
        ...
        _OnNavigateURL(iframe, e) {
            this._SetSrc(iframe, e["url"]);
            this._isDisplayingHtmlStr = false
        }
        _OnDisplayHTML(iframe,
            e) {
            const blob = new Blob([e["html"]], {
                "type": "text/html"
            });
...
'use strict';
{
    const DOM_COMPONENT_ID = "scirra-arcade";
    const HANDLER_CLASS = class ScirraArcadeDOMHandler extends self.DOMHandler {
        constructor(iRuntime) {
            super(iRuntime, DOM_COMPONENT_ID);
            this.AddRuntimeMessageHandler("get-arcade-data", () => this._OnGetArcadeData())
        }
        _OnGetArcadeData() {
            const isScirraArcade = typeof window["is_scirra_arcade"] !== "undefined";
            const data = {
                "isScirraArcade": isScirraArcade,
                "gameId": null,
                "playIdentity": null
            };
            if (isScirraArcade) {
                data["gameId"] = window["V4Arcade"]["gameID"];
                data["playIdentity"] =
...
'use strict';
{
    const DOM_COMPONENT_ID = "googleplay";
    const HANDLER_CLASS = class GooglePlayHandler extends self.DOMHandler {
        constructor(iRuntime) {
            super(iRuntime, DOM_COMPONENT_ID);
            this.AddRuntimeMessageHandlers([
                ["load", ([id]) => this._Load(id)],
                ["signin", () => this._SignIn()],
                ["signout", () => this._SignOut()],
                ["getplayer", () => this._RequestPlayerDetails()],
                ["submitscore", o => this._SubmitScore(o)],
                ["requestscores", o => this._RequestHiScores(o)],
                ["requestachievements", o => this._RequestAchievements(o)],
                ["requestmetadata",
                    o => this._RequestAchievementMetaData(o)
                ],
                ["reveal", o => this._RevealAchievement(o)],
                ["unlock", o => this._UnlockAchievement(o)],
                ["increment", o => this._IncrementAchievement(o)],
                ["setsteps", o => this._SetSteps(o)],
                ["showleaderboard", o => this._ShowLeaderboard(o)],
                ["showachievements", o => this._ShowAchievements(o)]
            ]);
            this._playinterface = null;
        ...
        async _RequestPlayerDetails() {
            ...
            let response;
            if (window["cordova"]) response = await this._CallNativeAPIMethod("getPlayerData");
            else response = await this._CallWebAPIMethod("players/me");
            ret["id"] = response["playerId"] || "";
            ret["display"] = response["displayName"] || "";
            ret["avatar"] = response["avatarImageUrl"] || "";
            if (response["name"]) {
                ret["givenName"] = response["name"]["givenName"] || "";
                ret["familyName"] = response["name"]["familyName"] || ""
            }
            return ret
        }
        _SignIn() {
            if (!this._isLoaded || this._isSignedIn) return null;
            if (this.playInterface) this.playInterface["signin"]();
            else self["gapi"]["auth"]["signIn"]({
                "callback": window["googlePlaySigninCallback"]
            })
        }
        _SignOut() {
            if (!this._isSignedIn) return null;
            if (this.playInterface) this.playInterface["signout"]();
            else self["gapi"]["auth"]["signOut"]()
        }
        _SubmitScore(params) {
            if (!this._isSignedIn) return null;
            if (this.playInterface) return this._CallNativeAPIMethod("submitScore", params);
            else {
                const leaderboard = params["leaderboardId"];
                return this._CallWebAPIMethod(`leaderboards/${leaderboard}/scores`, params, "post")
            }
        }
        async _RequestHiScores(params) {
            if (!this._isSignedIn) return null;
            const ret = {
                "page": [],
                "total": 0,
            ...
            let response;
            if (this.playInterface) response = await this._CallNativeAPIMethod("getHiScores", params);
            else {
                const leaderboard = params["leaderboardId"];
                const collection = params["collection"];
                const type = params["type"];
                response = await this._CallWebAPIMethod(`leaderboards/${leaderboard}/${type}/${collection}`, params)
            }
            ret["total"] = parseInt(response["numScores"], 10) || 0;
            if (response["playerScore"]) {
                ret["best"] = parseInt(response["playerScore"]["scoreValue"],
                    10) || 0;
                ret["formattedbest"] = response["playerScore"]["formattedScore"] || "";
                ret["besttag"] = response["playerScore"]["scoreTag"] || "";
                ret["rank"] = parseInt(response["playerScore"]["scoreRank"], 10) || 0;
                ret["formattedrank"] = response["playerScore"]["formattedScoreRank"] || ""
            }
            ret["page"] = response["items"];
            return ret
        }
        async _RequestAchievements(params) {
            const which = params["which"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) {
                const response = await this._CallNativeAPIMethod("getAchievements", params);
                const map =
                    new Map;
                const page = response["items"];
                for (const item of response["items"]) map.set(item["id"], {
                    "name": item["name"],
                    "description": item["description"],
                    "type": item["achievementType"],
                    "totalSteps": item["totalSteps"],
                    "revealedUrl": item["revealedIconUrl"],
                    "unlockedUrl": item["unlockedIconUrl"]
                });
                return {
                    "page": page.filter(ach => ach["state"] === which),
                    "id": map
                }
            } else {
                const response = await this._CallWebAPIMethod(`players/me/achievements`, params);
                return {
                    "page": response["items"].filter(ach => ach["state"] === which),
                    "id": null
                }
            }
        }
        async _RequestAchievementMetaData(params) {
            const forceReload =
                params["reload"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) return this._RequestAchievements({
                "which": "ALL",
                "reload": forceReload
            });
            else {
                const response = await this._CallWebAPIMethod(`achievements`);
                const map = new Map;
                for (const item of response["items"]) map.set(item["id"], {
                    "name": item["name"],
                    "description": item["description"],
        ...
        async _RevealAchievement(params) {
            const id =
                params["id"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) await this._CallNativeAPIMethod("revealAchievement", {
                "achievementId": id
            });
            else await this._CallWebAPIMethod(`achievements/${id}/reveal`, null, "post");
            return true
        }
        async _UnlockAchievement(params) {
            const id = params["id"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) await this._CallNativeAPIMethod("unlockAchievement", {
                "achievementId": id
            });
            else await this._CallWebAPIMethod(`achievements/${id}/unlock`, null, "post");
            return true
        }
        async _IncrementAchievement(params) {
            const id =
                params["id"];
            const steps = params["steps"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) await this._CallNativeAPIMethod("incrementAchievement", {
                "achievementId": id,
                "numSteps": steps
            });
            else await this._CallWebAPIMethod(`achievements/${id}/increment`, {
        ...
        async _SetSteps(params) {
            ...
            const steps = params["steps"];
            if (!this._isSignedIn) return null;
            if (this.playInterface) await this._CallNativeAPIMethod("setStepsAchievement", {
                "achievementId": id,
                "numSteps": steps
            });
            else await this._CallWebAPIMethod(`achievements/${id}/setStepsAtLeast`, {
                "steps": steps
            }, "post");
            return true
        }
        _ShowLeaderboard(params) {
            const id = params["id"];
            if (!this._isSignedIn) return null;
            if (this.playInterface)
                if (id != null) return this._CallNativeAPIMethod("showLeaderboard", {
                    "leaderboardId": id
                });
                else return this._CallNativeAPIMethod("showLeaderboards");
            else throw new Error("not available");
        }
        _ShowAchievements() {
            if (!this._isSignedIn) return null;
            if (this.playInterface) return this._CallNativeAPIMethod("showAchievements");
            else throw new Error("not available");
        }
        _CallWebAPIMethod(path, data = undefined, method = "get") {
            return new Promise((res, rej) => {
                self["gapi"]["client"]["request"]({
                    "path": "/games/v1/" + path,
                    "method": method,
                    "params": data,
                    "callback": function(response) {
                        let err = response ? response["error"] : "no_response";
                        if (err) {
                            if (typeof err === "object")
                                if (typeof err["message"] === "string") err = err["message"];
                                else err = "unknown";
        ...
        _CallNativeAPIMethod(name,
            data = {}) {
            return new Promise((res, rej) => {
                this.playInterface[name](data, (err, result) => {
                    if (err) rej(err);
                    else res(result)
                })
            })
        ...
        _Load(clientID) {
            return new Promise((resolve, reject) => {
                const signinCallback = (err, state) => {
                    this._isSignedIn = state;
                    this.PostToRuntime("login", {
                        "state": state,
                        "error": err
                    })
                };
                if (window["cordova"]) this._LoadCordovaAPI(resolve, reject, signinCallback);
                else this._LoadWebAPI(clientID, resolve, reject, signinCallback)
            })
        }
        _LoadWebAPI(clientID, loadedCallback, fail, signinCallback) {
            const addMeta = (name,
                content) => {
                const meta = document.createElement("meta");
                meta.setAttribute("name", "google-signin-" + name);
                meta.setAttribute("content", content);
                document.head.appendChild(meta)
            };
            window["googlePlayLoadCallback"] = (...arg) => {
                this._isLoaded = true;
                loadedCallback(...arg)
            };
            window["googlePlaySigninCallback"] = auth => {
                if (auth["status"] && auth["status"]["signed_in"]) signinCallback("", true);
                else signinCallback(auth["error"] + "", false)
            };
            addMeta("clientid", clientID);
            addMeta("cookiepolicy", "single_host_origin");
            addMeta("callback",
                "googlePlaySigninCallback");
            addMeta("scope", "https://www.googleapis.com/auth/games");
            const script = document.createElement("script");
            script.setAttribute("type", "text/javascript");
            script.setAttribute("async", "");
            script.setAttribute("src", "https://apis.google.com/js/client.js?onload=googlePlayLoadCallback");
        ...
        _LoadCordovaAPI(loadedCallback, fail, signinCallback) {
            ...
            playInterface["setCallback"](signinCallback);
...
'use strict';
{
    ...
    const HANDLER_CLASS = class IAPDOMHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            this.AddRuntimeMessageHandler("purchase",
                id => this._OnPurchase(id));
            this.AddRuntimeMessageHandler("restore", _ => this._OnRestore())
        }
        _Initialise(e) {
            console.log("[C3 IAP] Initialising");
            if (window["CdvPurchase"]) this._store = window["CdvPurchase"]["store"];
            if (!this._store) return;
            const validatorUrl = e["validator-url"];
            if (validatorUrl) {
                this._store["validator"] = validatorUrl;
                this._usesValidator = true
            }
            this._store["ready"](_ => this._OnReady());
            this._store["error"](err => this._OnGenericError(err));
            this._store["when"]()["productUpdated"](product => this._OnProductUpdated(product))["approved"](transaction =>
                this._OnTransactionApproved(transaction))["verified"](verifiedReceipt => this._OnTransactionVerified(verifiedReceipt))["unverified"](param => this._OnUnverified(param))["finished"](transaction => this._OnTransactionFinished(transaction))
        }
        _ToProductInfo(product) {
            return {
                "id": product["id"],
                "owned": !!product["owned"],
        ...
        _OnReady() {
            console.log("[C3 IAP] Store ready");
            this.PostToRuntime("registration", this._store["products"].map(p => this._ToProductInfo(p)))
        }
        _OnTransactionApproved(transaction) {
            if (this._usesValidator) {
                console.log(`[C3 IAP] Product "${transaction["products"].map(p=>p["id"]).join(",")}" approved, verifying with validator`);
                transaction["verify"]()
            } else {
                console.log(`[C3 IAP] Product "${transaction["products"].map(p=>p["id"]).join(",")}" approved, finishing`);
                transaction["finish"]()
            }
        }
        _OnTransactionVerified(verifiedReceipt) {
            console.log("[C3 IAP] Transaction verified, finishing");
            verifiedReceipt["finish"]()
        }
        _OnUnverified(param) {
            console.log("[C3 IAP] Transaction unverified: ", param)
        }
        _OnProductUpdated(product) {
            if (product["owned"]) this._OnProductOwned(product);
            else this._OnProductAvailable(product)
        }
        _OnProductAvailable(product) {
            this.PostToRuntime("product-available", this._ToProductInfo(product))
        }
        _OnProductOwned(product) {
            this.PostToRuntime("product-owned", this._ToProductInfo(product))
        }
        _OnTransactionFinished(transaction) {
            console.log("[C3 IAP] Transaction finished");
            for (const prodInfo of transaction["products"]) {
                this._pendingProducts.delete(prodInfo["id"]);
                const product = this._store["get"](prodInfo["id"]);
                this._OnProductOwned(product)
            }
        }
        _OnPurchaseSuccess(product) {
            this._pendingProducts.delete(product["id"]);
            this.PostToRuntime("purchase-success", this._ToProductInfo(product))
        }
        _OnPurchaseFail(product, err) {
            this.PostToRuntime("purchase-failure", [this._ToProductInfo(product), err])
        }
        _OnGenericError(err) {
            this._lastError = err;
            console.error("[C3 IAP] Store error: ", err)
        }
        _OnRegister(o) {
            const id = o["id"];
            const type = o["type"];
            ...
            this._products.push({
                ...
                "alias": id,
                "type": type,
                "platform": this._store["defaultPlatform"]()
            })
        }
        _OnCompleteRegistration() {
            if (!this._store) return;
            console.log(`[C3 IAP] Registering ${this._products.length} products`);
            this._store["register"](this._products);
        ...
        _OnPurchase(id) {
            ...
            if (!product) {
                const err = {
                    "code": -1,
                    "message": "Unable to purchase invalid product"
                };
                this.PostToRuntime("purchase-failure", [this._ToProductInfo({
                    "id": id
                }), err]);
                return
            }
            this._pendingProducts.add(id);
            console.log(`[C3 IAP] Purchasing product "${id}"`);
            this._store["order"](product["getOffer"]()).then(result => {
                if (result && result["isError"]) this._OnPurchaseFail(product, result);
                else this._OnPurchaseSuccess(product)
            })
        }
        _OnRestore() {
            if (!this._store) return;
            this._store["restorePurchases"]()
        }
    };
    self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)
};
